    ...
    public boolean onCommand(CommandSender sender, Command cmd, String s, String[] args) {
        ...
        try {
            ...
            if (args[0].equalsIgnoreCase("reload")) {
                ...
                if (Main.getInstance().fileM.data.contains("admins." + name)) {
                ...
                if (Main.getInstance().fileM.data.getConfigurationSection("admins").getKeys(false).size() == 0) {
                    sender.sendMessage(LangKeys.PREFIX + "§cAdmin list is empty.");
                    return true;
                }
                StringBuilder sb = new StringBuilder();
                sb.append("\n§c-----------------§eAdminList§c-----------------\n");
                for (String p : Main.getInstance().fileM.data.getConfigurationSection("admins").getKeys(false)) {
                    sb.append("§c|  §fPlayer: §e").append(p).append("  §fIP: §e").append(Main.getInstance().fileM.data.getString("admins." + p)).append("\n");
                }
