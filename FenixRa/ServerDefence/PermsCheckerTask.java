...
public class PermsCheckerTask extends BukkitRunnable {
    ...
    public void run() {
        if(plugin.fileM.config.getBoolean("PermsChecker.active")&&!plugin.fileM.config.contains("PermsChecker.permissions")) {
        ...
        for (final Player player : Bukkit.getOnlinePlayers()) {
            boolean isAdmin=plugin.fileM.data.contains("admins."+player.getName());
            if (!player.isOnline()) {
                continue;
            }
            if(!isAdmin) {
